...
@section('content')
    ...
    function initToast() {
        ...
        <div class="max-w-6xl mx-auto p-6 space-y-6" x-data="editForm({{ $employee->toJson() }})">
            ...
            <form id="editForm" x-on:submit.prevent="submit" class="space-y-6"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input name="leave_date" type="date"
                                class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2"
                                value="{{ old('leave_date', optional($employee->leave_date)->format('Y-m-d')) }}">
                            @error('leave_date')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="gender"
                                ...
                                @foreach (['Male', 'Female', 'Other'] as $g)
                                    <option value="{{ $g }}" @selected(old('gender', $employee->gender) === $g)>{{ $g }}
                                    </option>
                                @endforeach
                            </select>
                            @error('gender')
                        ...
                        <div>
                            ...
                            <input name="alternative_contact_number" type="text"
                                class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2"
                                value="{{ old('alternative_contact_number', $employee->alternative_contact_number) }}">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="status"
                                ...
                                @foreach (['Active', 'Inactive', 'Resigned'] as $s)
                                    <option value="{{ $s }}" @selected(old('status', $employee->status) === $s)>{{ $s }}
                                    </option>
                                @endforeach
                            </select>
                            @error('status')
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="5"
                                class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2">{{ old('address', $employee->address) }}</textarea>
                            @error('address')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="5"
                                class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2">{{ old('notes', $employee->notes) }}</textarea>
                            @error('notes')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                {{-- Documents: existing list + new dropzone --}}
                <div class="rounded-2xl p-6 bg-white/90 dark:bg-gray-800 shadow-lg" x-data="{ dragging: false }"
                    @dragover.prevent="dragging=true" @dragleave.prevent="dragging=false"
                    @drop.prevent="
                    dragging=false;
                 ...
                 ">
                    ...
                    {{-- Existing docs with remove checkboxes --}}
            ...
            {{-- Global delete modal (for convenience if you delete in edit page too) --}}
                ...
                submit() {
                    ...
                    xhr.onload = () => {
                        ...
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            safeToast('success', res.message ?? 'Employee updated');
