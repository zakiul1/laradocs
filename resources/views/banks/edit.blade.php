...
@section('content')
    @php
        ...
        // Try to resolve the current related company name from polymorphic relation (if you have one),
        // otherwise fall back to simple guess.
        $initialCompanyName = old('company_name');

        if ($initialCompanyName === null) {
            if (method_exists($bank, 'company') && $bank->company) {
                // If Bank has morphTo company()
                $related = $bank->company;

                if ($bank->company_type === Customer::class) {
                    $initialCompanyName = $related->name ?? '';
                } elseif ($bank->company_type === Company::class) {
                    $initialCompanyName = $related->company_name ?: $related->name ?? '';
                } else {
                    $initialCompanyName = '';
                }
            } else {
                // Old style: sometimes stored as plain text
                $initialCompanyName = $bank->company_name ?? '';
            }
        }
    @endphp
    ...
    <div class="max-w-6xl mx-auto p-6 space-y-6">
        <header class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Edit Bank</h1>
        ...
        <form id="bankEditForm" x-data="bankEdit()" x-init="init()" x-on:submit.prevent="submit"
            action="{{ route('admin.banks.update', $bank) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" x-model="type" required
                            ...
                            @foreach (['Customer Bank', 'Shipper Bank'] as $t)
                                <option value="{{ $t }}" @selected(old('type', $bank->type) === $t)>{{ $t }}
                                </option>
                            @endforeach
                        </select>
                        @error('type')
                ...
                <div class="space-y-4">
                    ...
                    {{-- Company (select-like dropdown with search) --}}
                    <div x-id="['company-dropdown']">
                        ...
                        <div class="relative" @click.away="openCompanyDropdown = false">
                            ...
                            <button type="button" :aria-disabled="!company_type"
                                ...
                                <span class="truncate"
                                    x-text="company_name
                                            || (company_type ? 'Select Company' : 'Select Type first')">
                                </span>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="!type">
                                Select <span class="font-semibold">Type</span> first to choose a company.
                            </p>

                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" x-show="company_name" x-cloak>
                                Selected:
                                <span class="font-medium" x-text="company_name"></span>
                            </p>
                        </div>
                ...
                <div class="space-y-4 md:col-span-1">
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="4"
                            class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2">{{ old('address', $bank->address) }}</textarea>
                        @error('address')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="note" rows="4"
                            class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2">{{ old('note', $bank->note) }}</textarea>
                        @error('note')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    @push('scripts')
        <script>
            function bankEdit() {
                return {
                    // main state
                    type: @json(old('type', $bank->type)),
                    company_id: @json(old('company_id', $bank->company_id)),
                    company_name: @json($initialCompanyName),
                    companyOptions: [],
                    ...
                    submit() {
                        ...
                        xhr.onload = () => {
                            if (xhr.status >= 200 && xhr.status < 300) {
                                ...
                                window.safeToast?.('success', res.message ?? 'Bank updated');
