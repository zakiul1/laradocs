...
@section('content')
    <div class="max-w-6xl mx-auto p-6 space-y-6">
        <header class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Edit Shipper</h1>
        ...
        <form id="shipperEditForm" x-data="shipperEdit()" x-on:submit.prevent="submit"
            action="{{ route('admin.shippers.update', $shipper) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input name="website" type="url"
                            class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2"
                            value="{{ old('website', $shipper->website) }}">
                        @error('website')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4 md:col-span-1">
                    <div>
                        ...
                        <textarea name="address" rows="6"
                            class="w-full rounded-2xl border border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-gray-800 px-4 py-2">{{ old('address', $shipper->address) }}</textarea>
                        @error('address')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    @push('scripts')
        <script>
            function shipperEdit() {
                return {
                    submit() {
                        const form = document.getElementById('shipperEditForm');
                        ...
                        xhr.onload = () => {
                            if (xhr.status >= 200 && xhr.status < 300) {
                                ...
                                window.safeToast?.('success', res.message ?? 'Shipper updated');
