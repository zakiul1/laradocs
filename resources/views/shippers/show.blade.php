...
@section('content')
    <div class="max-w-5xl mx-auto p-6 space-y-6">
        <header class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">{{ $shipper->name }}</h1>
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.shippers.edit', $shipper) }}"
                    class="px-4 py-2 rounded-2xl bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-100">
                    Edit
                </a>
                <form class="inline" method="POST" action="{{ route('admin.shippers.destroy', $shipper) }}"
                    onsubmit="return confirm('Delete this shipper?')">
                    @csrf
                    @method('DELETE')
                    <button
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    ...
                    <p
                        ...
                        @if ($shipper->website)
                            <a href="{{ $shipper->website }}" target="_blank"
                                class="text-indigo-600 hover:underline">{{ $shipper->website }}</a>
                        @else
                            —
                        @endif
                    </p>
                ...
                <div class="grid grid-cols-2 gap-3 md:col-span-2">
                    <div>
                        ...
                        <p class="font-medium">{{ $shipper->created_at?->format('Y-m-d H:i') }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 dark:text-gray-400">Updated</p>
                        <p class="font-medium">{{ $shipper->updated_at?->format('Y-m-d H:i') }}</p>
                    </div>
                    @if ($shipper->creator)
                        <div class="md:col-span-2">
                            <p class="text-gray-500 dark:text-gray-400">Created By</p>
                            <p class="font-medium">{{ $shipper->creator->name }}</p>
                        </div>
                    @endif
                </div>
            </div>
